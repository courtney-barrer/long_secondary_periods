...
pixelsize = 0.27 #0.6 
ud = 3.3 #3.3# 3.28 # in mas
fov = int(13*ud) #40
model = 'dipole'#'pmoired'#'dipole'
...
if 'pmoired' in model:
    pmoired_model = {"*,ud":ud} #,"*,f":1.0,"c,f":0.01,"c,x":0.0,"c,y":6.0,"c,ud":0.0} #{"p,f": 1, "r,x": 0, "r,y": 0, "p,ud": 5.462567068944827, "r,diamin": 10.3, "r,diamout": 50.1, "r,f": 1.0, "r,incl": 78.30669619040759, "r,projang": -36.67879133231321}
    ...
    theta_o = np.deg2rad(incl) #170
    phi_o =  np.deg2rad(projang) #172
    ...
    theta_rot, phi_rot = plot_util.rotate_to_observer_frame(theta, phi, theta_o, phi_o) #input here must be in radians
...
########################################
# writing model data to new fits files
########################################

test_mode = False # test mode copies the original file as the model file (for debugging)
print(f"\ntest_mode = {test_mode}\n")
...
for i, f in enumerate(obs_files):
    ...
    if not test_mode:
        ...
        print("[TEST MODE] Copying original as model for", f)
        orig_fname = os.path.basename(f)
        out_fname = os.path.join(model_pth, orig_fname.replace(".fits", ".model.fits"))
        hdul.writeto(out_fname, overwrite=True,output_verify='ignore')
        print("[SAVE]", out_fname)
        ...
        hdul.close()
########################################
# rreading in model & obs data then
# writing residual data to new fits files
########################################

# ----------------  ----------------
obs_glob = '/home/rtc/Documents/long_secondary_periods/data/pionier/data/*.fits'
model_dir = '/home/rtc/Documents/long_secondary_periods/dipole_residuals_pionier/model'
out_dir   = '/home/rtc/Documents/long_secondary_periods/dipole_residuals_pionier/residuals/'
os.makedirs(out_dir, exist_ok=True)

# Residual modes: 'ratio' or 'diff'
mode_v2     = 'ratio'   # V2 residual:  obs/model   or   obs - model
mode_t3amp  = 'diff'   # T3AMP residual: obs/model or   obs - model
# T3PHI residual is always (obs - model) wrapped to [-180,180]
# -------------------------------------------
...
def _propagate_ratio(val_o, err_o, val_m, err_m, ratio):
    # σ_r ≈ r * sqrt( (σo/Vo)^2 + (σm/Vm)^2 )
    rel2 = np.zeros_like(ratio, dtype=float)
    m = np.isfinite(ratio) & np.isfinite(val_o) & (val_o != 0) & np.isfinite(val_m) & (val_m != 0)
    rel2[m] = (np.where(err_o[m]>0, (err_o[m]/val_o[m])**2, 0.0) +
               np.where(err_m[m]>0, (err_m[m]/val_m[m])**2, 0.0))
    out = np.abs(ratio) * np.sqrt(rel2)
    out[~np.isfinite(out)] = np.nan
    return out

def _propagate_diff(err_o, err_m):
    # σ_d = sqrt(σo^2 + σm^2)
    return np.sqrt(np.square(err_o) + np.square(err_m))

obs_files = sorted(glob.glob(obs_glob))
for f in obs_files:
    base = os.path.basename(f)
    model_path = os.path.join(model_dir, base.replace('.fits', '.model.fits'))
    if not os.path.exists(model_path):
        print(f"[SKIP] Model file not found for {base}")
        continue

    # open files
    hdul = fits.open(f)
    hdul_mod = fits.open(model_path)

    # work in a copy of the original so headers/flags stay intact
    #hdul = fits.HDUList([hdu.copy() for hdu in hdul_obs])

    # ---------- OI_VIS2 ----------
    if "OI_VIS2" in hdul and "OI_VIS2" in hdul_mod:
        obs_v2  = np.array(hdul["OI_VIS2"].data["VIS2DATA"], dtype=float)
        obs_e2  = np.array(hdul["OI_VIS2"].data["VIS2ERR"],  dtype=float)
        mod_v2  = np.array(hdul_mod["OI_VIS2"].data["VIS2DATA"], dtype=float)
        mod_e2  = np.array(hdul_mod["OI_VIS2"].data["VIS2ERR"],  dtype=float)

        # align shapes defensively
        if obs_v2.shape != mod_v2.shape:
            print(f"[WARN] V2 shape mismatch in {base}: {obs_v2.shape} vs {mod_v2.shape}")
        nrow = min(obs_v2.shape[0], mod_v2.shape[0])
        obs_v2, obs_e2, mod_v2, mod_e2 = obs_v2[:nrow], obs_e2[:nrow], mod_v2[:nrow], mod_e2[:nrow]

        if mode_v2 == 'ratio':
            res_v2 = _safe_div(obs_v2, mod_v2)
            res_e2 = _propagate_ratio(obs_v2, obs_e2, mod_v2, mod_e2, res_v2)
        else:  # 'diff'
            res_v2 = obs_v2 - mod_v2
            res_e2 = _propagate_diff(obs_e2, mod_e2)

        hdul["OI_VIS2"].data["VIS2DATA"][:nrow] = res_v2
        hdul["OI_VIS2"].data["VIS2ERR"] [:nrow] = res_e2
        # keep UCOORD/VCOORD/STA_INDEX/FLAG from original

    # ---------- OI_T3 ----------
    if "OI_T3" in hdul and "OI_T3" in hdul_mod:
        obs_amp  = np.array(hdul["OI_T3"].data["T3AMP"],   dtype=float)
        obs_eamp = np.array(hdul["OI_T3"].data["T3AMPERR"],dtype=float)
        obs_phi  = np.array(hdul["OI_T3"].data["T3PHI"],   dtype=float)
        obs_ephi = np.array(hdul["OI_T3"].data["T3PHIERR"],dtype=float)

        mod_amp  = np.array(hdul_mod["OI_T3"].data["T3AMP"],   dtype=float)
        mod_eamp = np.array(hdul_mod["OI_T3"].data["T3AMPERR"],dtype=float)
        mod_phi  = np.array(hdul_mod["OI_T3"].data["T3PHI"],   dtype=float)
        mod_ephi = np.array(hdul_mod["OI_T3"].data["T3PHIERR"],dtype=float)

        # align shapes defensively
        if obs_amp.shape != mod_amp.shape:
            print(f"[WARN] T3AMP shape mismatch in {base}: {obs_amp.shape} vs {mod_amp.shape}")
        if obs_phi.shape != mod_phi.shape:
            print(f"[WARN] T3PHI shape mismatch in {base}: {obs_phi.shape} vs {mod_phi.shape}")
        nrow = min(obs_amp.shape[0], mod_amp.shape[0], obs_phi.shape[0], mod_phi.shape[0])
        obs_amp, obs_eamp, mod_amp, mod_eamp = obs_amp[:nrow], obs_eamp[:nrow], mod_amp[:nrow], mod_eamp[:nrow]
        obs_phi, obs_ephi, mod_phi, mod_ephi = obs_phi[:nrow], obs_ephi[:nrow], mod_phi[:nrow], mod_ephi[:nrow]

        # amplitude residual
        if mode_t3amp == 'ratio':
            res_amp = _safe_div(obs_amp, mod_amp)
            res_eam = _propagate_ratio(obs_amp, obs_eamp, mod_amp, mod_eamp, res_amp)
        else:  # 'diff'
            res_amp = obs_amp - mod_amp
            res_eam = _propagate_diff(obs_eamp, mod_eamp)

        # phase residual (wrap to [-180,180])
        res_phi = _wrap_deg(obs_phi - mod_phi)
        # simple error: quadrature
        res_eph = _propagate_diff(obs_ephi, mod_ephi)

        hdul["OI_T3"].data["T3AMP"][:nrow]    = res_amp
        hdul["OI_T3"].data["T3AMPERR"][:nrow] = res_eam
        hdul["OI_T3"].data["T3PHI"][:nrow]    = res_phi
        hdul["OI_T3"].data["T3PHIERR"][:nrow] = res_eph
        # keep U1COORD/V1COORD/U2COORD/V2COORD/STA_INDEX/FLAG from original

    # # annotate history
    # try:
    #     hdul[0].header.add_history(f"Residuals written from {base} and {os.path.basename(model_path)}")
    #     hdul[0].header.add_history(f"VIS2 mode={mode_v2}; T3AMP mode={mode_t3amp}; T3PHI=wrapped(obs-model)")
    # except Exception:
    #     pass

    out_path = os.path.join(out_dir, base.replace('.fits', '.res.fits'))
...
# ### Basic check 
obs_files = glob.glob('/home/rtc/Documents/long_secondary_periods/data/pionier/data/*.fits') #sorted(glob.glob(obs_glob))
mod_files = glob.glob('/home/rtc/Documents/long_secondary_periods/dipole_residuals_pionier/model/*.model.fits') #sorted(glob.glob(model_dir + '/*.model.fits'))
res_files =  glob.glob('/home/rtc/Documents/long_secondary_periods/dipole_residuals_pionier/residuals/*.res.fits') #sorted(glob.glob(out_dir + '/*.res.fits'))

oi_obs = pmoired.OI(obs_files)
oi_mod = pmoired.OI(mod_files)
oi_res = pmoired.OI(res_files)
