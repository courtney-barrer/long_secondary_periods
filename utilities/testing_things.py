...
input_str = f"ymira -initial=Dirac -regul={regul} -pixelsize={pixelsize}mas -fov={fov}mas -wavemin={wavemin*1e3}nm -wavemax={wavemax*1e3}nm -mu={mu} -tau={tau} -eta={eta} -gtol={gtol} -ftol={ftol} -flux=1 -min=0 -bootstrap=1 -save_dirty_map -save_dirty_beam -use_vis=none -use_vis2={use_vis2} -overwrite -use_t3=none {input_file} {output_imreco_file}"
...
def write_oifits(fake_data, output_filename):
    ...
    primary_hdu.header['MJD'] = fake_data[0]['OI_VIS2']['A0B2']['MJD'][0]  # Example: First MJD
