...
def bandmask_from_oi_wavelength(hdul, ranges_um, every=1, offset=0):
    """Return {INSNAME: keep_mask} from OI_WAVELENGTH.EFF_WAVE (µm),
    optionally decimating to keep every Nth channel within the ranges.

    Parameters
    ----------
    hdul : fits.HDUList
    ranges_um : list[(float,float)]
        Inclusive [lo, hi] µm intervals.
    every : int
        Keep every Nth channel inside the union of ranges (>=1).
    offset : int
        Starting offset in [0, every-1] applied within the kept-index list.
    """
    if every < 1:
        raise ValueError("--every must be >= 1")
    if offset < 0 or offset >= max(1, every):
    ...
    for h in hdul:
        if h.name == "OI_WAVELENGTH":
            ...
            # Optional decimation within the True indices
            if every > 1:
                idx = np.flatnonzero(keep)
                if idx.size:
                    decimated = np.zeros_like(keep)
                    decimated[idx[offset::every]] = True
                    keep = decimated
...
def apply_flag_mask(hdul, masks):
    ...
    for h in hdul:
        if h.name in ("OI_VIS2", "OI_VIS", "OI_T3", "OI_FLUX"):
            ...
            # Ensure 2D (NROW, NWAVE); OI_T3 shares this convention
...
def write_band_filtered(infile, outfile, ranges_um, every=1, offset=0):
    with fits.open(infile, mode="readonly") as hdul:
        hdul_out = fits.HDUList([h.copy() for h in hdul])

    masks = bandmask_from_oi_wavelength(hdul_out, ranges_um, every=every, offset=offset)
    if not masks:
        raise RuntimeError(f"No OI_WAVELENGTH tables found in {infile}")

    # Sanity: at least one channel kept for any INSNAME we’ll touch
    if all((~m).all() for m in masks.values()):
        raise RuntimeError("Range/decimation removed all channels for all INSNAMEs.")
    ...
    for lo, hi in ranges_um:
        hdr0.add_history(f"Flagged channels outside [{lo:.4f}, {hi:.4f}] um")
    if every > 1:
        hdr0.add_history(f"Decimated wavelengths: kept every {every} with offset {offset}")
...
def main():
    ap = argparse.ArgumentParser(
        description="Flag OIFITS channels outside a wavelength band (and optionally decimate) and save copies."
    )
    ap.add_argument("--ins", required=True, help="instrument key for output path (e.g. pionier, gravity, matisse_L)")
    ap.add_argument("--band", required=True, help="label for folder name (e.g. L, M, K)")
    ...
    ap.add_argument("--every", type=int, default=1, help="keep every Nth channel inside the range (default: 1)")
    ap.add_argument("--offset", type=int, default=0, help="start offset in [0, N-1] (default: 0)")
    ...
    if args.every > 1:
        tag += f"_every{args.every}_off{args.offset}"

    print(f"Saving to: {out_dir}")
    print(f"Band: [{args.wvl_min:.4f}, {args.wvl_max:.4f}] µm  tag: {tag}")
    if args.every > 1:
        print(f"Decimation: every={args.every}, offset={args.offset}")
    rng = [(args.wvl_min, args.wvl_max)]

    for ii, f in enumerate(files, 1):
        base = os.path.basename(f)
        out_name = (base[:-5] if base.lower().endswith(".fits") else base) + f"_{tag}.fits"
        out_path = os.path.join(out_dir, out_name)
        write_band_filtered(f, out_path, rng, every=args.every, offset=args.offset)
...
# K band, keep all channels in [2.10, 2.11] µm
python /home/rtc/Documents/long_secondary_periods/data/wvl_filter_oifits.py \
  --ins gravity --band K \
  --wvl-min 2.100 --wvl-max 2.110 --every 5 --offset 1 \
  --input-glob "/home/rtc/Documents/long_secondary_periods/data/gravity/data/*.fits"

# K band, keep every 3rd channel (offset 0) in [2.10, 2.20] µm
python wvl_filter_oifits.py \
  --ins gravity --band K \
  --wvl-min 2.100 --wvl-max 2.200 \
  --every 3 --offset 0 \
  --input-glob "/home/rtc/Documents/long_secondary_periods/data/gravity/data/*.fits"

# MATISSE L band, decimate: every 2nd channel, start at offset 1
python wvl_filter_oifits.py \
  --ins matisse --band L \
...
# #e.g. to filter K band 
# ############################
# python /home/rtc/Documents/long_secondary_periods/data/wvl_filter_oifits.py \
#   --ins gravity --band K \
#   --wvl-min 2.100 --wvl-max 2.110 \
#   --input-glob "/home/rtc/Documents/long_secondary_periods/data/gravity/data/*.fits" 
