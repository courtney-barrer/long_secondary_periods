...
instr = 'GRAVITY'
keyword = 'K'
fig_path = f'/home/rtc/Documents/long_secondary_periods/image_reconstruction/image_reco/{instr}_{keyword}/{what_imgs}_IMAGES/'
...
for prior in ['Dirac']:#['UD','random','Dirac']:
    ...
    for w in np.unique(res_df['spectral_feature']):

        # create a new folder f or wavelength if it doesn't exist
        if not os.path.exists(fig_path + f'{w}/'):
            os.makedirs(fig_path + f'{w}/')

        tmp_fig_path = fig_path + f'{w}/'
        for reg in np.unique(res_df['regul']):
            filt = (res_df['spectral_feature'] == w) & (res_df['regul'] == reg)
            if what_imgs == 'ALL':
