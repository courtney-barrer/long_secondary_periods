...
files2image = glob.glob( '/home/rtc/Documents/long_secondary_periods/data/merged_files/RT_Pav_PIONIER_MERGED_2022-04-29.fits' )# data_path + f'*PIONI*.fits') 
...
#pionier 
pixelsize = 1.6e-6/120 * 1e3 * 3600 * 180/3.14  /5
...
wavegrid = [1.4] #np.arange(8,13,0.5)
"""
regularisation 
...
ins_folder = 'PIONIER_H' # where to save output. e.g. f'/home/rtc/Documents/long_secondary_periods/image_reconstruction/image_reco/{ins_folder}/'
mu_grid = np.array([0] + list(np.logspace(0,6,10)) ) # the relative strength of the prior (compared to the data)
# compactness parameters
gamma_grid = np.linspace(1,100,7) #full width at half maximum (FWHM) of the prior distribution of light. 
# for hyperbolic
tau_grid = np.logspace(-6,0,7) # the edge threshold - sets the transition between the quadratic (l2) and the linear (l1) behavior of total variation (TV)
...
for prior_type in ['random', 'Dirac', 'UD']:    
    ...
    if (prior_type !='random') & (prior_type !='Dirac') :
        prior_path = f'/home/rtc/Documents/long_secondary_periods/image_reconstruction/image_reco/{ins_folder}/priors/{prior_type}/' #'/home/rtc/Documents/long_secondary_periods/image_reconstruction/image_reco/MATISSE_N/priors/'

    for wavemin in wavegrid: #micron - must specify this unit in input ()
        print(wavemin)
        wavemax = wavemin + 0.4 #np.median(np.diff(wavegrid )) #micron
        ...
        for regul in ['compactness','hyperbolic']:
            ...
            output_path = f'/home/rtc/Documents/long_secondary_periods/image_reconstruction/image_reco/{ins_folder}/MIRA_prior-{prior_type}_wave-{wavemin}-{wavemax}um-reg-{regul}_fov{fov}_pixSc{round(pixelsize,1)}/'
    ...
    telem.to_csv(f'/home/rtc/Documents/long_secondary_periods/image_reconstruction/image_reco/{ins_folder}/SUMMARY_RESULTS_IMG_RECO_{prior_type}_{instr}_{keyword}.csv',index=False, header=True) 
